...
public class ToHopMonService {
    ...
    public List<ToHopMon> searchAnd(String matohop, int page) {
        String m = (matohop == null) ? "" : matohop.trim();
        if (m.isBlank()) return repository.findAll(page, PAGE_SIZE);
        return repository.searchAnd(m, page, PAGE_SIZE);
    }

    public int getTotalPagesAnd(String matohop) {
        String m = (matohop == null) ? "" : matohop.trim();
        long count = m.isBlank() ? repository.countAll() : repository.countSearchAnd(m);
